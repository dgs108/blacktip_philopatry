...
rda_indv <- function(rda, n){
  ...
  lc <- as.data.frame(rda$CCA$u) %>%
    ...
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_LC")))
  # scale by eigenvalues
  scale <- scores(rda, 1:n, display = "lc", scaling = "sites") %>%
    as.data.frame() %>%
    rownames_to_column("hiseq_id") %>%
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_LC_SCALED1"))) %>%
    left_join(lc)
  # symmetric scaling by square root of eigenvalues
  lc <- scores(rda, 1:n, display = "lc", scaling = "symmetric") %>%
    as.data.frame() %>%
    rownames_to_column("hiseq_id") %>%
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_LC_SCALED3"))) %>%
    left_join(scale)
  # extract individual loadings (WA)
  wa <- as.data.frame(rda$CCA$wa) %>%
    rownames_to_column("hiseq_id") %>%
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_WA")))
  # scale by eigenvalues
  scale <- scores(rda, 1:n, display = "wa", scaling = "sites") %>%
    as.data.frame() %>%
    rownames_to_column("hiseq_id") %>%
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_WA_SCALED1"))) %>%
    left_join(wa)
  # symmetric scaling by square root of eigenvalues
  wa <- scores(rda, 1:n, display = "wa", scaling = "symmetric") %>%
    as.data.frame() %>%
    rownames_to_column("hiseq_id") %>%
    magrittr::set_names(c("hiseq_id", paste0("RDA", seq(1:n), "_WA_SCALED3"))) %>%
    left_join(scale)
  # combine all loadings
  loadings <- bind_cols(lc, wa) %>%
    rename(hiseq_id = hiseq_id...1)
 #   select(-hiseq_id...8)
